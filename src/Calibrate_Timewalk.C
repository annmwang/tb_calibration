...
int main(int argc, char* argv[]){
  ...
  if(!user_output){
    ...
    output_name = input_name+"_TimewalkCalib.root";
    sprintf(outputFileName,"%s.root",inputFileName);
  } else {
    output_name = string(outputFileName);
  }

  TChain* tree = new TChain("TimeWalk_fit");
  tree->AddFile(inputFileName);

  TimewalkFitBase* base = new TimewalkFitBase(tree);
  ...
  vector<vector<double> > vminPDO;     // min Pdo (start of timeWalk)
  vector<vector<double> > vmaxPDO;     // max Pdo (end of timeWalk)
  vector<vector<vector<double> > > vmeanTDO;    // mean TDO
  vector<vector<vector<double> > > vmeanTDOerr; // mean TDO err
  vector<vector<vector<int> > >    vPDO;      // Pdo Value
  ...
  for(int i = 0; i < N; i++){
    ...
    if(MMFE8VMM_to_index.count(pair<int,int>(MMFE8,VMM)) == 0){
      ...
      vminPDO.push_back(vector<double>());
      vmaxPDO.push_back(vector<double>());
      vmeanTDO.push_back(vector<vector<double> >());
      vmeanTDOerr.push_back(vector<vector<double> >());
      vPDO.push_back(vector<vector<int> >());
    }
    ...
    if(vCH_to_index[index].count(CH) == 0){
      ...
      vminPDO[index].push_back(0);
      vmaxPDO[index].push_back(1024);
      vmeanTDO[index].push_back(vector<double>());
      vmeanTDOerr[index].push_back(vector<double>());
      vPDO[index].push_back(vector<int>());
    }

    // CH index
    int cindex = vCH_to_index[index][CH];

    vPDO[index][cindex]=*base->Pdo;
    vminPDO[index][cindex]=base->min_pdo;
    vmaxPDO[index][cindex]=base->max_pdo;
    vmeanTDO[index][cindex]=*base->meanTdo;
    vmeanTDOerr[index][cindex]=*base->sigmaTdo;
    
  }

  int Nindex = vMMFE8.size();

  vector<vector<TGraphErrors*> > vgraph_Timewalk;
  for(int i = 0; i < Nindex; i++){
    vgraph_Timewalk.push_back(vector<TGraphErrors*>());
    int Ncindex = vCH[i].size();
    for(int c = 0; c < Ncindex; c++){
      int Npoint = vPDO[i][c].size();
      double PDO[Npoint];
      double meanTDO[Npoint];
      double meanTDOerr[Npoint];
      vgraph_Timewalk[i].push_back(new TGraphErrors());
      int nGraph=vgraph_Timewalk[i].size()-1;
      for(int p = 0; p < Npoint; p++){
        if(vmeanTDOerr[i][c][p]<15){
          vgraph_Timewalk[i][nGraph]->SetPoint(vgraph_Timewalk[i][nGraph]->GetN(),vPDO[i][c][p],vmeanTDO[i][c][p]);
          vgraph_Timewalk[i][nGraph]->SetPointError(vgraph_Timewalk[i][nGraph]->GetN()-1,0,vmeanTDOerr[i][c][p]);
        } 
	//PDO[p] = vPDO[i][c][p];
	//meanTDO[p] = vmeanTDO[i][c][p];
	//meanTDOerr[p] = vmeanTDOerr[i][c][p];
      }
      //vgraph_Timewalk[i].push_back(new TGraphErrors(Npoint, PDO, meanTDO, 0, meanTDOerr));
    }
  }
  ...
  fout->mkdir("TimewalkFit_plots");
  fout->cd("TimewalkFit_plots");
  for(int i = 0; i < Nindex; i++){
    char sfold[50];
    sprintf(sfold, "TimewalkFit_plots/Board%d_VMM%d", vMMFE8[i], vVMM[i]);
    fout->mkdir(sfold);
    fout->cd(sfold);
    ...
    for(int c = 0; c < Ncindex; c++){
      ...
      char scan[50];
      sprintf(scan, "c_TDOvPDO_Board%d_VMM%d_CH%d", vMMFE8[i], vVMM[i], vCH[i][c]);
      TCanvas* can = Plot_Graph(scan, vgraph_Timewalk[i][c], "PDO", "TDO", stitle);
      can->Write();
      delete can;
    }
  }
 ...
 // }
 //       // look for where the TDO value jumps abruptly
 //       if (last_TDO > 0){
 //         if ( fabs(vmeanTDO[i][c][p] - last_TDO) > max_jump ) {
 //           i_max_jump = vDelay[i][c][p];
 //           max_jump = fabs(vmeanTDO[i][c][p] - last_TDO);

 //         }
 //       }
 //       last_TDO = vmeanTDO[i][c][p];
 //     }
 ...
 //   diff = vDelay[i][c][p]+25.-max_delay;
 // else
 ...
 // 	meanTDOerr[p] = vmeanTDOerr[i][c][p];
 // 	if(vDelay[i][c][p] < max_delay){
 //   meanD[p] = step*max_diff-step*(vDelay[i][c][p]+25.-max_delay);
 // } else {
 //   meanD[p] = step*max_diff-step*(vDelay[i][c][p]-max_delay);
 // }
 // 	meanDerr[p] = 0.;
 //     }
